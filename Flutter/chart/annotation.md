...
The [`x`]() and [`y`]() values can be specified with axis units or pixel units, and these can be identified by using [`coordinateUnit`]() property. When logicalPixel is speciied, the annotation will be placed with respect to pixel values whereas point is specified, then the annotation will be placed with respect to series point values.
...
You can add multiple annotations to the Chart by adding multiple idgets to the [`annotations`]() property. as depicted in below code snippet.
